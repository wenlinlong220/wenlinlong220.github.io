...
整车分为上位机和下位机，上位机采用Edgeboard，下位机采用TC264，另外采用百度提供的框架进行YOLO模型的训练以识别路径上的特殊元素和路段。车模需要在行驶的过程中行驶过急转弯、S弯、环岛和十字路口等复杂路段，除此之外，还需要避让路上的行人和路障，在临时停车区、充电区以及餐饮区完成相应的任务，并且最终能够到达终点。

本人在项目中最主要负责的部分是上位机视觉代码的内容，在技术实现中有具体解释，除此之外也涉及其他的部分内容。
